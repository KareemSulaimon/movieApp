...
function Home() {
  const { data, handleClick } = useStateContext();
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10 mt-10 justify-center items-center w-11/12 m-auto' data-testid="movies-grid">
        {
          data && movies && movies.length > 0 ? (
            ...
            <Link key={movie.id} to={`/movies/${movie.id}`} onClick={() => handleClick(movie.id)} className='flex justify-center items-center' data-testid={`movie-link-${movie.id}`}>
                  <MovieCard movieData={movie} onClick={() => handleClick(movie.id)} />
              </Link>
            ))
          ) : (
