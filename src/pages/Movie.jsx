...
function Movie() {
    const { movie, baseImageUrl } = useStateContext();
    const [movieDetail, setMovieDetail] = useState(null);

  useEffect(() => {
    // Update movieDetail when movie is updated
    setMovieDetail(movie);
  }, [movie]);

   console.log(movie)
   const runtime = movie && movieDetail && movieDetail.runtime / 60
   const watchTime = Math.round(runtime)
   let vote_count   = Math.ceil(movie && movieDetail && movieDetail.popularity / 1000)
          ...
          <div className="flex flex-col xs:flex-wrap w-4/5 items-center">
              <div className='flex w-full cursor-pointer gap-10 overflow-hidden rounded-xl relative'>
                  <img
                      ...
                      src={ movieDetail && baseImageUrl + movieDetail.backdrop_path}
                      alt="Movie Poster"
            ...
            <div className='flex flex-col sm:flex-row items-center mt-2 w-full justify-between'>
              <div className='flex flex-wrap w-full gap-3  items-center'>
                <h1 className='flex flex-wrap text-gray-600 font-black text-xl mb-6'>{ movieDetail && movieDetail.original_title}</h1>
                <div className='flex gap-3 items-center font-black text-xl mb-6'> 
                    <h1 className=' w-2 h-2 rounded-full bg-gray-600'></h1>
                     <h1 className='text-xl text-gray-600'>
                        { movie && movieDetail && movieDetail.release_date ? movieDetail.release_date.slice(0, 4) : "Not yet released"}
                       </h1>
                   <h1 className=' w-2 h-2 rounded-full bg-gray-600'></h1>
                </div>
                 <div className='flex gap-3 items-center font-black text-2xl mb-6'> 
                    ...
                    <h1 className=' w-2 h-2 rounded-full bg-gray-600'></h1>
                    <h1 className='text-xl text-gray-600'>{watchTime}h {movieDetail && movieDetail.runtime % 2}m</h1>
                  </div>
                
            
            {movie && movieDetail &&  movieDetail.genres ? movieDetail.genres.map(genre => (
            ...
            font-bold rounded text-red-800 text-sm mb-6" data-testid="genre-button">
              ...
              <div className='flex items-center'>
                ...
                <h3 className='bg-white px-2 py-2 rounded text-red-700 font-black text-xl mb-6'>
                  { movie && movieDetail && movieDetail.vote_count ? Math.ceil( movieDetail.vote_count /1000) : "Non"}
                </h3>
            ...
            <div className='flex flex-col w-full sm:w-3/5 gap-1/5 justify-between items-start'>
                  {/* <div className='flex flex-col w-3/5 justify-between items-start'> */}
                      <p className='w-full text-16 text-gray-800'>{movieDetail && movieDetail.overview}</p>     
...
// function Movie() {
//   const { data, movie,baseImageUrl } = useStateContext(); 
//   const [movieDetail, setMovieDetail] = useState(null)
...
//   if (movie ) {
//   setMovieDetail(movie)
//   } else if (!movie || movie === null) {
//     setMovieDetail("is Loading")
//   }
...
//               <img
//               className="w-full h-[500px] object-cover"
//               src={baseImageUrl + movieDetail.backdrop_path}
//               alt="Movie Poster"
//               data-testid="movie-poster"
//             />
//         <div className='flex flex-col items-center absolute top-28 left-[40%]'>
...
//           <div className='flex flex-col sm:flex-row items-center mt-2 w-full justify-between'>
//           <div className='flex flex-wrap w-full gap-3  items-center'>
//           <h1 className='flex flex-wrap text-gray-600 font-black text-xl mb-6'>{movieDetail.original_title}</h1>
//           <div className='flex gap-3 items-center font-black text-xl mb-6'> 
//             <h1 className=' w-2 h-2 rounded-full bg-gray-600'></h1>
//               <h1 className='text-xl text-gray-600'>{movieDetail.release_date.slice(0, 4)}</h1>
//               <h1 className=' w-2 h-2 rounded-full bg-gray-600'></h1>
//             </div>
//           <div className='flex gap-3 items-center font-black text-2xl mb-6'> 
//               <h1 className='text-xl text-gray-600'>PG 13</h1>
...
//               <button className="px-2 py-1 bg-white border border-solid border-red-200
//               font-bold rounded text-red-800 text-xl mb-6" data-testid="genre-button">
//                 {genre}
//              </button>
//          )) }
